...
cassiopeia_guest_post_day_point_reset();
...
function cassiopeia_theme_menu_tree(&$variables) {
	if (!empty($variables['theme_hook_original']) && $variables['theme_hook_original'] == 'menu_tree__main_menu') {
...
function cassiopeia_theme_status_messages($variables) {
	...
	if (count($message_list) > 0) {
		$output .= '<div class="" id="message-model">
...
function cassiopeia_theme_status_messages_old($variables) {
	$display = $variables['display'];
	$output = '';
	...
	if (count($message_list) > 0) {
		$output .= '<div class="" id="message-model" tabindex="-1" role="dialog" aria-labelledby="message-model-label">
