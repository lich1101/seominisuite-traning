...
var executeTab = null;
var executeTabDelay = 12e4;
var executeTabTimeout = 0;
var request = null;
var sender = null;
var spinUrlRequest = "";
var tabRemoves = {};
// chrome.storage.local.set({'executeTab': null});
// chrome.storage.local.set({'request': null});
// chrome.storage.local.set({'sender': null});
function resetAll(){
    chrome.storage.local.set({'executeTab': null});
    chrome.storage.local.set({'request': null});
    chrome.storage.local.set({'sender': null});
}
function setTabId(editorExtensionId,tab) {
    // chrome.storage.sync.set({editorExtensionId: editorExtensionId}, function() {
    //     console.log('Value is set to ' + editorExtensionId);
    // });
    console.log("tab",tab);
...
chrome.runtime.onMessage.addListener(async function (_request, _sender, sendResponse) {
    ...
    // chrome.storage.sync.set({_request_: _request}, function() {
    // console.log('Value is set to ' + value);
    // });
    if (_request.type === "captcha_dos") {
        if (sender && executeTab && executeTab.id == _sender.tab.id) {
            let responseObj = {};
        ...
        chrome.storage.local.set({'request': _request});
        chrome.storage.local.set({'sender': _sender});
        request = _request;
        sender = _sender;
        sendResponse({response: ""});
        if (_request.type === "KeyCheck") {
            console.log(":request",request.currentObj.key);
            // console.log(request.currentObj.domain);
            let linkUrl = _request.currentObj.searchEngine + "/search?q=" + _request.currentObj.key + "&num=100&hl=" + _request.currentObj.language + "&start=0&ie=utf-8&oe=utf-8";
            ...
            let linkUrl = "https://www.google.com/search?q=site:" + _request.currentObj.source + "&hl=vi";
            ...
            if (response.url.includes("sorry/index?continue=https://www.google.com")) {
                chrome.tabs.create({url: response.url, active: false},async function (tab) {
                    ...
                    //     },
                    //     args: [editorExtensionId],
                    //     files: ['js/tab_captcha.min.js']
                    // });


                    // chrome.tabs.executeScript(tab.id, {code: 'var editorExtensionId = "' + chrome.runtime.id + '";'}, function (result) {
                    //     chrome.tabs.executeScript(tab.id, {file: "js/tab_captcha.min.js"}, function (result) {
                    //         executeTab = tab;
                    //         if (executeTabTimeout != 0) {
                    //             clearTimeout(executeTabTimeout)
                    //         }
                ...
                responseObj["currentObj"] = request.currentObj;
                responseObj["content"] = new Array(responseText);
                cassiopeiaSendResponse(request.data, responseObj, sender, "CheckBackLinkIndexed");
                resetAll();
                request = null;
                sender = null;
                executeTab = null
            ...
            const onTimeout = timeout(resolve => resolve("waiting"), 6e5);
...
chrome.tabs.onUpdated.addListener(async function (tabId, changeInfo, tab) {
    if(request!==null&&request.currentObj!==null){
        console.log(":request",request.currentObj.key);
    }else{
...
function afterUpdateTab(tabId, changeInfo, tab) {
    // var _executeTab;
    // var _request;
    chrome.storage.local.get('executeTab', function (result) {
        var _executeTab = result.executeTab;
        chrome.storage.local.get('request', function (result) {
            var _request = result.request;
            chrome.storage.local.get(['sender'], function(result) {
                var _sender = result.sender;
                if (changeInfo.hasOwnProperty("url")) {
                    if (changeInfo.url.includes("search?q")) {
                        console.log("_executeTab 2",_executeTab)
                        if (_executeTab !== null && tabId === _executeTab.id) {
                            console.log("exe");
                            chrome.scripting.executeScript(
                                {
                                    target: {tabId: tabId},
                            ...
                            console.log('Value currently is ' + tabId+" executeTab "+_executeTab);
                            // });
                        }
                    }else{
